...
"""
Lancet Global News Classifier Qwen3.5 SFT Training

Examples:
  python classification/train_lancet_a100v2.py
  python classification/train_lancet_a100v2.py --model 4b --resp-only
  python classification/train_lancet_a100v2.py --merge-and-push
"""
...
parser = argparse.ArgumentParser(description="Lancet News Classifier SFT Training")
parser.add_argument("--model", type=str, default="9b", choices=MODELS.keys(),
...
DATASET_REPO = f"{HF_USERNAME}/lancet-classifier-v2-sft"
MODEL_REPO = f"{HF_USERNAME}/lancet-classifier-v2-qwen3.5-{MODEL_SIZE}"
MAX_SEQ_LENGTH = 4096
...
OUTPUT_DIR = f"lancet-classifier-v2-qwen3.5-{MODEL_SIZE}"
...
config = SFTConfig(
    ...
    run_name=f"lancet-classifier-v2-qwen3.5-{MODEL_SIZE}",
)
...
if args.merge_and_push:
    MERGED_REPO = f"{HF_USERNAME}/lancet-classifier-v2-qwen3.5-{MODEL_SIZE}-merged"
